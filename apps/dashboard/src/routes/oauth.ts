...
app.get('/oauth/callback', async (c) => {
  ...
  if (error) {
    ...
    return c.html(
      layout('OAuth Error', `<h1>OAuth Error</h1><p class="badge badge--red">${errorMsg}</p><p><a href="/ext">&larr; Back to Plugins</a></p>`),
    );
  }
  ...
  try {
    ...
    if (!res.ok || !data.success) {
      return c.html(
        layout('OAuth Error', `<h1>OAuth Error</h1><p class="badge badge--red">${data.error ?? 'Token exchange failed'}</p><p><a href="/ext">&larr; Back to Plugins</a></p>`),
      );
    }
